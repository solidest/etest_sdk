...
## 入门教程简介
我们在本入门教程中将会完成一个基于ETestDev开发工具套件运行的一个演示项目的例子,为了适应用户的操作习惯ETestDev所有编辑界面均支持操作快捷键,复制(ctrl+c)粘贴(ctrl+v)撤销(ctrl+z)恢复(ctrl+y).数据库文件存放的位置为系统默认的用户程序数据目录
+ windows系统`C:\Users\user_name\AppData\Roaming\etest_dev\`
+ linux或mac`/var/user_name`

### 项目库
项目库是本系统内的所有项目进行一个统一的管理，支持新建项目、重命名项目、删除项目、收索项目、项目导入、项目导出、项目排序等一系列操作,页面标题可以展示出设计器的版本号
...
### 设备接口
设备接口是对本项目下的所有的设备、设备的接口、以及接口类型参数名称的一个统一的管理。支持设备、接口的新增编辑以及接口参数的配置等操作,同时支持文件的快速复制与导出单个ETL文件功能.以下所有操作均在设备接口菜单栏下进行操作
...
### 连接拓扑
连接拓扑是表示项目下的设备接口的拓扑关系，支持拓扑新增、编辑、删除、设备的映射、接口的连接、运行时接口的绑定等信息,同时支持文件的快速复制与导出单个ETL文件功能.链接设备列表可以进行颜色区分
...
### 通信协议
通信协议表示项目下设备之间通信所使用的协议，支持协议以及子协议的编辑、复制、设置属性等多种操作,同时支持文件的快速复制与导出单个ETL文件功能.
...
+ 单个协议的编写完整，即可进行测试,可以解包成16进制,2进制并且现实其自动赋值属性
![协议测试](./photo/prot_test.png)


### 监控面板
监控面板主要是监控测试程序的的实时输出，并且把实时状态以图像化的界面展示出来。分为图形化子面板(按钮、文字显示、状态指示、文字输入、多行文本输入、数字输入、开关按钮、复选框、单选框、下拉选择、复合选择、滑块)，界面子面板(仪表、曲线、散点).同时支持文件的快速复制与导出单个ETL文件功能.
...
+ 复合选择举例说明(符合选择支持选择，同时也支持手动输入)
    ...
    ```
+ `注: 复选框控件绑定值为true或false,开关空间绑定值为0或1`
#### 图形子面板的参数配置
+ 仪表盘参数说明
    ```yml
    series:
...
### 执行用例
执行用例是对项目中的用例进行执行的操作，用例执行时需要选择拓扑连接、监控面板、执行方式、实时调度周期等参数。输出参数为Yaml格式，在独立的窗口中进行，代码编辑窗口下面有详细的输出结果,同时支持文件的快速复制功能.
...
#### 状态机生成器
+ 编写yaml格式配置文件,可以自动生成状态图和执行代码
+ 配置文件格式为数组对象,每个对象必须设置三个属性`state when then`
