...
+ 数据流类型的接口可设置参数`before_recv`，用于指定接收过滤器的函数名
+ 接收过滤器为用户自定义函数，用于在接收之前对数据进行过滤
+ 过滤器函数的输入参数为buffer句柄
+ 接收过滤器返回`-1`时，丢弃已接收数据
+ 接收过滤器返回`0`时，接收全部数据
+ 接收过滤器返回一个正整数`p`时，从第`p`个字节开始接收
+ 接收过滤器返回一个整数`p1`和另外一个数字`p2`时，接收第`p1`至`p2`之间的字节
+ 返回值`p2`可以为负数，表示倒数第n个字节
...
![协议段的位序](https://solidest.github.io/etest_sdk/bitorder.png)
...
+ `length`属性用于设置字符串类型协议段的字节长度
+ `endwith`属性用于设置字符串类型协议段的结尾标识符，打包时自动添加结尾符，解包时自动删除结尾符
+ `length`和`endwith`需至少设置一个属性，同时设置时`endwith`被解析器忽略
