...
+ 数据流类型的接口可设置参数`before_recv`，用于指定接口过滤器的函数名
+ 接口过滤器函数的输入参数为buffer句柄
+ 接口过滤器返回`-1`时将，丢弃已接收数据
+ 接口过滤器返回`0`时将，接收全部数据
+ 接口过滤器返回一个正整数`p`时，从第`p`个字节开始接收
+ 接口过滤器返回一个正整数`p1`和另外一个数字`p2`时，接收第`p1`至`p2`之间的字节
