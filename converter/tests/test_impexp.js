...
const impexp = require('../src/impexp');
const helper = require('../src/helper');
const { assert } = require('console');

function test_import_exprot_proj(idx_obj, out_path1, outpath2) {
    let proj = impexp.import_etl(idx_obj, path.resolve('./test_proj'), shortid.generate());
    assert(proj);
...
const idx_obj = yaml.safeLoad(helper.read_text('index.yml', './test_proj'));
